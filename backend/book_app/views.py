...
def get_paragraphs(request):
    ...
    text_id = ObjectId(request.GET.get('text_id'))
...
def get_translation(request):
    ...
    vocabulary_term = VocabularyTerm.objects.filter(**request.GET).first()

    if vocabulary_term is None:
        english = request.GET.get('english')

        print("ENGLISHs", english)
        ...
        voice_over = synthesize_text(english)
        ...
        vocabulary_term_serializer.save()
    else:
